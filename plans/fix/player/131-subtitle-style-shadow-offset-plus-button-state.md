# Subtitle Style Shadow Offset Plus Button State

## Source Plan
- `plans/player/subtitle-style-shadow-offset-plus-button-work-plan.md`

## Gap Analysis
The current E2E clicks `player.subtitle_style.shadow_offset.plus` only in a broad presence loop and does not assert state, displayed value, repeated clamp at `10.0`, close/reopen persistence, or subtitle overlay rendering. See `apps/player/src-tauri/tests/plan_ui_e2e.rs:444`.
...
- Tests must verify Shadow Offset plus changes `subtitle_style.shadow_offset`.
- Subtitle rendering must use `subtitle_style.shadow_offset`.
- Tests must verify the displayed Shadow Offset value updates after plus.
...
## Required Test Shape
- Click Shadow Offset plus and assert state, visible value, and automation value.
- Repeatedly click plus and assert `shadow_offset == 10.0`.
...
## Verification
- `cargo test -p tench-player --test plan_ui_e2e subtitle_style_shadow_offset_plus`
