# Subtitle Style Position Plus Button State

## Source Plan
- `plans/player/subtitle-style-position-plus-button-work-plan.md`

## Gap Analysis
The current E2E clicks `player.subtitle_style.position.plus` once and asserts `subtitle_style.position` increased, but it does not assert the displayed value label, repeated clamp at `1.0`, close/reopen persistence, or subtitle overlay rendering. See `apps/player/src-tauri/tests/plan_ui_e2e.rs:427`.
...
- Tests must verify the displayed Position value updates after plus.
...
## Required Test Shape
- Click Position plus and assert state, visible value, and automation value.
- Repeatedly click plus and assert `position == 1.0`.
...
## Verification
- `cargo test -p tench-player --test plan_ui_e2e subtitle_style_position_plus`
