# Subtitle Style Font Size Minus Button State

## Source Plan
- `plans/player/subtitle-style-font-size-minus-button-work-plan.md`

## Gap Analysis
The current E2E clicks `player.subtitle_style.font_size.minus` once and asserts `subtitle_style.font_size` decreased, but it does not assert the displayed value label, repeated clamp at `8.0`, close/reopen persistence, or subtitle overlay rendering. See `apps/player/src-tauri/tests/plan_ui_e2e.rs:403`.
...
- Tests must verify the displayed Font Size value updates after minus.
...
## Required Test Shape
- Click Font Size minus and assert state, visible value, and automation value.
- Repeatedly click minus and assert the value clamps at `8.0`.
...
## Verification
- `cargo test -p tench-player --test plan_ui_e2e subtitle_style_font_size_minus`
